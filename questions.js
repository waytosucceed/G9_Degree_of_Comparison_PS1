...
const uniqueKey = 'G9_Degree of Comparison_MCQs'
...
function loadOptionQuestion (randomQuestion, optionsElement, index) {
  ...
  randomQuestion.options.forEach(function (option, idx) {
    ...
    if (option.image) {
      ...
      optionImage.style.maxWidth = '320px'
...
function displayResults() {
  ...
  // Track which stories have been displayed and their question ranges
  var displayedStories = new Set()
  var storyPages = [] // Will store story page content
  var questionPages = [] // Will store question page content

  // Group questions by story
  var questionGroups = []
  var currentGroup = { story: null, questions: [] }
  
  for (var j = 0; j < questions.length; j++) {
    var quesgroup = questions[j]
    
    if (quesgroup.story !== currentGroup.story) {
      // New story found, save previous group if it has questions
      if (currentGroup.questions.length > 0) {
        questionGroups.push(currentGroup)
      }
      // Start new group
      currentGroup = { story: quesgroup.story, questions: [j] }
    } else {
      // Same story, add to current group
      currentGroup.questions.push(j)
    }
  }
  ...
  questionGroups.forEach(function(group, groupIndex) {
    // Create story page if story exists
    if (group.story) {
      var storyPage = "<div class='question-page' style='display: " +
        (allPages.length === 0 ? 'block' : 'none') +
        ";'>" +
        "<h2>Page " + (allPages.length + 1) + "</h2>" +
        "<div style=' padding: 10px; margin: 10px 0; border-radius: 8px; border-left: 4px solid #699e19;'>" +
        "<strong style='color: #699e19; font-size: 1em;'>Story for Questions " + 
        (group.questions[0] + 1) + " to " + (group.questions[group.questions.length - 1] + 1) + ":</strong><br><br>" +
        "<div style='margin-top: 10px; line-height: 1.6; font-size: 0.75em;'>" + group.story + "</div>" +
        "</div>" +
        "</div>"
      allPages.push(storyPage)
    }
    
    // Create question pages (5 questions per page)
    var questionsPerPage = 5
    var numQuestionPages = Math.ceil(group.questions.length / questionsPerPage)
    
    for (var p = 0; p < numQuestionPages; p++) {
      var start = p * questionsPerPage
      var end = Math.min(start + questionsPerPage, group.questions.length)
      
      var pageDiv = "<div class='question-page' style='display: " +
        (allPages.length === 0 ? 'block' : 'none') +
        ";'><h2>Page " + (allPages.length + 1) + "</h2>"
      
      for (var idx = start; idx < end; idx++) {
        var j = group.questions[idx]
        var quesgroup = questions[j]
